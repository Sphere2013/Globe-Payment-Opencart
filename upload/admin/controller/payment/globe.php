...
class ControllerPaymentGlobe extends Controller {
	...
	private function validate() {
        ...
        if (!$this->request->post['globe_rpc_username']) {
            $this->error['username'] = $this->language->get('error_username');
        }
        if (!$this->request->post['globe_rpc_address']) {
            $this->error['address'] = $this->language->get('error_address');
        }
        if (!$this->request->post['globe_rpc_password']) {
            $this->error['password'] = $this->language->get('error_password');
        }
        if (!$this->request->post['globe_rpc_port']) {
            $this->error['port'] = $this->language->get('error_port');
        }
        if (!$this->request->post['globe_prefix']) {
            $this->error['prefix'] = $this->language->get('error_prefix');
        }
        if (!$this->request->post['globe_countdown_timer']) {
	...
	public function index() {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('payment/globe', 'token=' . $this->session->data['token'], 'SSL'),
            'separator' => ' :: '
        );
		...
		$max = $result->num_rows;
		$globe_total_in_db = 0;
		$globe_address_in_db = 0;
		for($i = 0;$i < $max;$i++) {
			if($rows[$i]["Field"] == "globe_total") {
				$globe_total_in_db = 1;
			}
			if($rows[$i]["Field"] == "globe_address") {
				$globe_address_in_db = 1;
			}
		}
		if(!$globe_total_in_db) {
			$this->db->query("ALTER TABLE ". DB_PREFIX ."order ADD globe_total DOUBLE AFTER currency_value;");
		}
		if(!$globe_address_in_db) {
			$this->db->query("ALTER TABLE ". DB_PREFIX ."order ADD globe_address VARCHAR(34) AFTER globe_total;");
		}
